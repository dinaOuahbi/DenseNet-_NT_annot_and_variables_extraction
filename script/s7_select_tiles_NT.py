...
SCAN_reinhard = "/work/shared/ptbc/CNN_Pancreas_V2/database/TCGA/macenko/"
svsa = "/work/shared/ptbc/CNN_Pancreas_V2/results/TCGA/StromaVSautre_DB10/StromaVSautre/"


def select_tiles_NT(slide):
    myPrediction = pd.read_csv(f"{svsa}{slide}.csv", sep=",", header=None)
    ...
    myPrediction["pred_tile"].value_counts().plot(
        kind="bar", color="cyan", edgecolor="red"
    )
    for i in range(myPrediction["pred_tile"].value_counts().shape[0]):
        plt.annotate(
            myPrediction["pred_tile"].value_counts()[i],
    ...
    plt.grid(True)
    plt.savefig(
        f"{snn}N_Tselect/pred_class_dist_png/Pred_classes_distribution_{slide}.png"
    )
